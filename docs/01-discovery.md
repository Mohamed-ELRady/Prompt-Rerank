...
**Project:** PromptPolish *(working title — easy to rename later; nothing in the architecture depends on it)*
...
- **FR-C2** The analysis is injected into the rewrite meta-prompt so the LLM fixes *identified* weaknesses rather than rewriting blindly.
